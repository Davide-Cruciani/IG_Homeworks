...
vec3 Shade( Material mtl, vec3 position, vec3 normal, vec3 view )
{
	...
	for ( int i=0; i<NUM_LIGHTS; ++i ) {
		...
		shadowRay.pos = position + normal * shadowBias;
		HitInfo info;
...
bool IntersectRay( inout HitInfo hit, Ray ray )
{
	...
	for ( int i=0; i<NUM_SPHERES; ++i ) {
		...
		if (delta>=0.0){
			...
			float t = 1e30;
			if(t1>intersectionBias && t1<t) t=t1;
			if(t2>intersectionBias && t2<t) t=t2;
...
vec4 RayTracer( Ray ray )
{
	...
	if ( IntersectRay( hit, ray ) ) {
		...
		vec3 lastPosition = hit.position + hit.normal*reflectionBias;
		vec3 lastDir = reflect(-view, hit.normal);
